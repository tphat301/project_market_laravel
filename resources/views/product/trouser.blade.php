...
@section("title", "Danh sách quần nữ")
@section("content")
<div class="container box-cart">
    ...
    <h3 class="title-skirt">Quần thời trang</h3>
    <div class="skirt-main">
        <div class="filter">
            <p>Hiển thị tất cả {{ $trouserProduct->total() }} kết quả</p>
        ...
        <ul class="list-skirt">
            @foreach ($trouserProduct as $item)
                <li>
        ...
        {{ $trouserProduct->links() }}
    </div>
    <script async type="text/javascript">
        const selectElement = document.getElementById("filter");
        function filter() {
            if(selectElement.value == "asc") {
                return window.location = "{{ url('danh-sach-quan-nu?filter=asc') }}";
            }
            if(selectElement.value == "desc") {
                return window.location = "{{ url('danh-sach-quan-nu?filter=desc') }}";
            }
        }
    </script>
</div>
